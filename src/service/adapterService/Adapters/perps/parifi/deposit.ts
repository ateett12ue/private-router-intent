...
import { abiEncode, GetBatchHandlerAddress, MAX_UINT_256 } from "../../../../../utils";
class ParifiDepositAdapter extends AdapterBase
{
    ...
    ComposeCalldata(adapter: AdapterIdParamsResponse): Promise<CalldataResponse>
    {
        return new Promise(async (resolve, reject) =>
        {
            try
            {
                ...
                const amountIn = MAX_UINT_256;
                const calldata = abiEncode(
                    ["address", "uint256"],
                    [adapterOptions.receiverAddress, amountIn]
                );
...
export default ParifiDepositAdapter;
