...
import { service as BenqiApi } from "../../../../../serviceclients/adapterPriceApi/stake/BenqiApi";
...
class BenqiStakeAdapter extends AdapterBase
{
    ...
    GetAdapterEstimates(data: AdapterQuotationParams): Promise<AdapterQuotationApiResponse>
    {
        return new Promise(async (resolve, reject) =>
        {
            try
            {
                const getExchangePrice = await BenqiApi.PriceApiAvalance(String(data.amount));
                const finalAmount = Number(
                    formatEther(String(getExchangePrice), data.destDecimal || 18)
                );
                const weiAmount = getExchangePrice;
        ...
        return new Promise(async (resolve, reject) =>
        {
            try
            {
                ...
                const calldata = abiEncode(
                    ["address", "uint256"],
                    [adapterOptions.receiverAddress, amountMax256]
                );
...
export default BenqiStakeAdapter;
